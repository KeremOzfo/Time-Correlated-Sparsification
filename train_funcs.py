...
def train(args, device):
    ...
    testEpoch = 0
    runs = math.ceil(N_s*args.num_epoch/(args.bs*num_client))
    for run in range(runs):
        epoch = math.floor(run * args.bs * num_client / N_s)
        for cl in range(num_client):
